...
pub fn save_image(image: &ProcessedImage, path: &Path, #[allow(unused_variables)] quality: u8) -> Result<()> {
    let ext = path
        .extension()
        .and_then(|e| e.to_str())
        .unwrap_or("")
    ...
    match ext.as_str() {
        ...
        _ => {
            ...
            {
                anyhow::bail!(
                    "JPEG output requires the `jpg` feature (recompile with --features jpg)"
                );
            }
        }
    }
