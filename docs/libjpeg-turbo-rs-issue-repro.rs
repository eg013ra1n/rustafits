// Trailing-MCU padding divergence at 4:2:0 — upstream issue #362.
//
// Originally a bug reproduction against libjpeg-turbo-rs 0.6.x; kept as the
// re-check to run whenever the pinned encoder version is bumped. Fixed upstream
// in 0.7.0; see libjpeg-turbo-rs-issue.md for the result table and for why an
// aarch64-only run proves nothing (it is byte-clean on NEON either way).
//
// Not a build target — copy into a scratch crate:
//
//   [dependencies]
//   libjpeg-turbo-rs = "=<version under test>"
//   turbojpeg        = "1.5"   # C libjpeg-turbo, the reference
//
// Run with `cargo run --release` AND `cargo run --release --target <an x86_64 target>`.
// Every '=' in the 4:2:0 row is a pass; any '#' is a divergence from C.
