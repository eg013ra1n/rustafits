...
*independent* decoder, not the decode side of `libjpeg-turbo-rs`, so a
symmetric bug there cannot pass a self-round-trip.

## Release

No system build dependencies: every dep is pure Rust, so packaging recipes must
not reintroduce cmake/nasm. CI builds Linux x86_64 on tag push (`v*.*.*`). macOS is built manually (see `BUILD_MACOS.md`). Packages: Homebrew tap, AUR, Debian, RPM spec.
